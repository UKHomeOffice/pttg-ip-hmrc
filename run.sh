...
#   done
  ...
  awk '/-----BEGIN CERTIFICATE-----/{filename="acpca-b-"NR; print filename}; {print >filename}' /certs/ca-certificates.crt
  
  for file in acpca-b*; do
    echo "Process certificate in ${file}"
    keytool -import -alias "${file}" -file "${file}" -keystore ./truststore.jks -noprompt -storepass changeit -trustcacerts
    echo "Now remove cert file ${file}"
    rm "${file}"
  done  
